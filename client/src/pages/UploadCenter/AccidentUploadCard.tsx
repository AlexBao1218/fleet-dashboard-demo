...
import { uploadAccident } from '@client/src/api/accident';
import type { AccidentUploadResponse } from '@shared/accident';

export interface AccidentUploadCardProps {
...
const AccidentUploadCard = ({ onUploaded }: AccidentUploadCardProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState<boolean>(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [error, setError] = useState<string>('');
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [result, setResult] = useState<AccidentUploadResponse | null>(null);
  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (uploadError || !data) {
        logger.error(`Accident 文件上传失败: ${JSON.stringify(uploadError)}`);
        throw new Error('文件上传失败，请稍后重试');
      }
      const parsed = await uploadAccident({
      ...
      logger.error(`Accident 上传解析失败: ${JSON.stringify(err)}`);
  ...
  return (
    <section className="rounded-md border border-border bg-card p-6">
      ...
      <div className="mt-4 rounded-md border border-border p-4">
        ...
        >
          ...
          <p className="text-xs text-foreground">
            Click or drag a .xlsx file here
          </p>
          <input
            ref={inputRef}
            type="file"
        ...
        {result ? (
          <div className="mt-2 rounded-md border border-border p-3">
            <div className="grid grid-cols-4 gap-2 font-mono text-xs tabular-nums">
              <div>
            ...
            <p className="mt-2 font-mono text-xs text-foreground">
              进入统计范围 {result.inScopeCount}{' '}
              条（交通事故且责任方为 Own/Third），不计入 {result.outOfScopeCount}{' '}
              条
            </p>
            {result.warnings ? (
      ...
      <p className="mt-4 text-sm text-muted-foreground">{ACCIDENT_HINT}</p>
    </section>
  );
};

export default AccidentUploadCard;
