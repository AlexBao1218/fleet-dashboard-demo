...
import { uploadHistory } from '@client/src/api/fuel-transaction';
import {
  SERVER_PROCESS_LIMIT_BYTES,
  buildFileSizeErrorMessage,
} from '@client/src/utils/upload-error';
import type { HistoryUploadResponse } from '@shared/fuel-transaction';

export interface HistoryUploadCardProps {
  onUploaded: () => Promise<void>;
}

const HISTORY_HINT =
  '一次性导入历史油卡交易。文件必须是标准 11 列格式（列名与 fuel_transaction 字段相同）。此入口只用于历史迁移，日常上传请用上面的入口。';

const isCsvFile = (file: File): boolean => {
  return file.name.toLowerCase().endsWith('.csv');
};

export const HistoryUploadCard = ({ onUploaded }: HistoryUploadCardProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState<boolean>(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [error, setError] = useState<string>('');
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [result, setResult] = useState<HistoryUploadResponse | null>(null);
  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (uploadError || !data) {
        logger.error(`history 文件上传失败: ${JSON.stringify(uploadError)}`);
        throw new Error('文件上传失败，请稍后重试');
      }
      const parsed = await uploadHistory({
      ...
      logger.error(`history 上传解析失败: ${JSON.stringify(err)}`);
  ...
  return (
    <section className="rounded-md border border-border bg-card p-6">
      ...
      >
        ...
        <p className="text-sm text-foreground">Click or drag a .csv file here</p>
        <input
          ref={inputRef}
          type="file"
          accept=".csv"
      ...
      {error ? (
        <div className="mt-2 rounded-md p-3" style={{ backgroundColor: 'hsl(0 60% 96%)' }}>
          <p className="text-sm" style={{ color: 'hsl(0 60% 34%)' }}>{error}</p>
        </div>
      ) : null}
      {selectedFile ? (
      ...
      <p className="mt-4 text-sm text-muted-foreground">{HISTORY_HINT}</p>
    </section>
  );
};

export default HistoryUploadCard;
