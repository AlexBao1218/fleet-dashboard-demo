...
import { uploadFuelCard } from '@client/src/api/fuel-transaction';
import {
  SERVER_PROCESS_LIMIT_BYTES,
  buildFileSizeErrorMessage,
} from '@client/src/utils/upload-error';
import type { FuelCardUploadResponse } from '@shared/fuel-transaction';

export interface FuelCardUploadCardProps {
...
export const FuelCardUploadCard = ({ onUploaded }: FuelCardUploadCardProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState<boolean>(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [error, setError] = useState<string>('');
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [result, setResult] = useState<FuelCardUploadResponse | null>(null);
  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (uploadError || !data) {
        logger.error(`fuel_card 文件上传失败: ${JSON.stringify(uploadError)}`);
        throw new Error('文件上传失败，请稍后重试');
      }
      const parsed = await uploadFuelCard({
      ...
      logger.error(`fuel_card 上传解析失败: ${JSON.stringify(err)}`);
  ...
  return (
    <section className="rounded-md border border-border bg-card p-6">
      ...
      <p className="mt-4 text-sm text-muted-foreground">{FUEL_CARD_HINT}</p>
    </section>
  );
};

export default FuelCardUploadCard;
