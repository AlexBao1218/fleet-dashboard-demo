...
import { parseFleetUpload } from '@client/src/api/fleet-vehicle';
import type { FleetUploadParseResponse } from '@shared/fleet-vehicle';

export interface FleetUploadCardProps {
  onUploaded: () => Promise<void>;
}

const FLEET_HINT =
  '上传车队主表。只读取 sheet \'DRAFT-Fleet List\'。有变动时整份重新上传，同车牌会被覆盖。';

const isFleetFile = (file: File): boolean => {
  const name = file.name.toLowerCase();
  return name.endsWith('.xlsx') || name.endsWith('.xlsm');
};

export const FleetUploadCard = ({ onUploaded }: FleetUploadCardProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState<boolean>(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [error, setError] = useState<string>('');
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [result, setResult] = useState<FleetUploadParseResponse | null>(null);
  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (uploadError || !data) {
        logger.error(`fleet 文件上传失败: ${JSON.stringify(uploadError)}`);
        throw new Error('文件上传失败，请稍后重试');
      }
      const parsed = await parseFleetUpload({
      ...
      logger.error(`fleet 上传解析失败: ${JSON.stringify(err)}`);
  ...
  return (
    <section className="rounded-md border border-border bg-card p-6">
      ...
      >
        ...
        <p className="text-sm text-foreground">Click or drag an .xlsx / .xlsm file here</p>
        <input
          ref={inputRef}
          type="file"
          accept=".xlsx,.xlsm"
      ...
      <p className="mt-4 text-sm text-muted-foreground">{FLEET_HINT}</p>
    </section>
  );
};

export default FleetUploadCard;
